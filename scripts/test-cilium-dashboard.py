...
KNOWN_GOOD_CHART_VERSION = "1.20.1"
...
class CiliumDashboardTests(unittest.TestCase):
    def test_known_good_chart_is_pinned_in_every_install_path(self):
        release = yaml.safe_load(RELEASE.read_text())
        self.assertEqual(KNOWN_GOOD_CHART_VERSION, release["spec"]["chart"]["spec"]["version"])
        for relative_path in (
            ...
            "bootstrap/templates/kubernetes/bootstrap/talos/apps/helmfile.yaml.j2",
        ):
            self.assertIn(f"version: {KNOWN_GOOD_CHART_VERSION}", (ROOT / relative_path).read_text(), relative_path)

    def test_regressed_chart_is_excluded_from_renovate(self):
        config = RENOVATE.read_text()
        marker = "'Block Cilium 1.20.2 BTF regression'"
        self.assertIn(marker, config)
        marker_offset = config.index(marker)
        rule = config[config.rfind("{", 0, marker_offset) : config.find("\n    },", marker_offset)]
        self.assertIn("matchDatasources: [\n        'helm',", rule)
        self.assertIn("matchPackageNames: [\n        'cilium',", rule)
        self.assertIn("allowedVersions: '!/^1\\\\.20\\\\.2$/'", rule)
    ...
    def test_provenance_and_canonical_validation_are_wired(self):
        ...
        for expected in (
            "7d68cfb394f2960e10aa72e76d0d51e66c1b2ebc",
            CHART_SHA256,
            "Cilium Metrics",
            "Apache-2.0",
        ):
