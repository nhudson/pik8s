...
4. If a Warning Event contains `out of bounds memory access`, the 1Password SDK client is permanently wedged by the [known upstream cache failure](https://github.com/external-secrets/external-secrets/issues/6941). ExternalSecret status contains only a sanitized provider-error message, so the least-privilege `external-secrets-sdk-recovery` CronJob detects the exact signature in Events and rolls the controller Deployment so it creates a clean client within two minutes.
5. The recovery job records the processed Event revision and enforces a ten-minute cooldown. New Event revisions seen during the cooldown are recorded without another rollout; continued failures after the cooldown can trigger one more recovery.
6. Verify the recovery Job completed, the controller restarted, and every ExternalSecret refreshed successfully. Do not rotate provider credentials or edit Secret values for this failure mode.
7. Recovery signal: the ExternalSecret and provider store are Ready and the consumer remains healthy.
...
The relay connects to the operator-managed egress Service, not directly to the public internet. The original vault-backed HTTPS URL still supplies the TLS hostname, Host header, and webhook path; only the TCP destination changes. The certificate remains verified. Keep the existing egress NetworkPolicy in place during this transport rollout; tighten it to DNS and the named Tailscale proxy pod on port 443 only after signed delivery has passed.
