...
4. If a Warning Event on an ExternalSecret or the `1password` ClusterSecretStore contains `out of bounds memory access`, the 1Password SDK client is permanently wedged by the [known upstream cache failure](https://github.com/external-secrets/external-secrets/issues/6941). ExternalSecret events may only report that the store is not ready, and ExternalSecret status contains only a sanitized provider-error message. The least-privilege `external-secrets-sdk-recovery` CronJob scans Events for that exact signature (including store Warning Events) and rolls the controller Deployment so it creates a clean client within two minutes. It ignores warnings on other stores; its existing cluster-wide Event list permission needs no RBAC expansion.
